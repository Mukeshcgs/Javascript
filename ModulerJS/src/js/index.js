...
$(document).on("click", ".o-list-table_item .toggle-click", function () {
  var $el = $(this);
  var $parent = $el.parent('.list-toggle-click');
  if (!$parent.hasClass('active')) {
    $parent.addClass('active');
    var $sib = $el.siblings(".content-toggle").slideDown("fast");
    var opened = $('.list-toggle-click').find(".content-toggle").not($sib);
    if (opened.length > 0) {
      opened.slideUp('fast');
    ...
    $el.siblings(".content-toggle").slideUp("fast");
  }
  function onloadCallback() {
    ...
    recaptchas.each(function (i, g) {
      var $g = $(g);
      var id = $g.attr('id');
      var k = $g.data('sitekey');

      var nbChildren = $g.children().length;
      if (nbChildren === 0) {
        var gid = grecaptcha.render(id, { 'sitekey': k });
        $g.attr('data-gid', gid);
      }
    });
  }
